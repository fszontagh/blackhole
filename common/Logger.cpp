...
Logger::Logger() : outputStream_(&std::cout) {}
...
void Logger::log(LogLevel level, const std::string &message) {
  ...
  if (outputStream_ == &std::cout || outputStream_ == &std::cerr)
    ss << getColorCode(level);
#endif

  ss << "[" << timeStr << "] " << levelToString(level) << ": " << message;
...
std::string Logger::getResetCode() { return "\033[0m"; }
